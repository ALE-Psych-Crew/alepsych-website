...
<head>
  ...
  <style>
    :root {
      ...
      --accent-2: #f6a15a;
      --accent-2-soft: rgba(242, 139, 60, 0.2);
    ...
    * {
    ...
    .content a:focus-visible {
      color: var(--accent-2);
      border-bottom-color: rgba(242, 139, 60, 0.5);
    }
    ...
    .content .callout-icon {
      ...
      font-weight: 700;
      color: var(--accent-2);
      border: 1px solid rgba(242, 139, 60, 0.6);
      background: rgba(242, 139, 60, 0.2);
    ...
    .content .callout a {
      color: #ffb066;
      border-bottom-color: rgba(255, 176, 102, 0.6);
    }

    .content .callout a:hover,
    .content .callout a:focus-visible {
      color: var(--accent-2);
      border-bottom-color: rgba(242, 139, 60, 0.8);
    }
...
<body data-meta-section="cookbook" data-meta-title="ALE Psych Engine Cookbook" data-meta-description="Documentation for ALE Psych Engine.">
  <header>
    <div class="topbar">
      <div class="brand">
  ...
  <script>
    ...
    const uiCopy = {
      en: {
        ...
        pageDescription: 'Guides and documentation for building with ALE Psych Engine.',
      ...
      es: {
        ...
        pageDescription: 'Guías y documentación para construir con ALE Psych Engine.',
    ...
    function renderMarkdown(markdown) {
      ...
      const withCallouts = decorateCallouts(rendered);
      const sanitized = DOMPurify.sanitize(withCallouts, { USE_PROFILES: { html: true } });
    ...
    function decorateCallouts(html) {
      ...
      blockquotes.forEach((blockquote) => {
        ...
        const calloutMatch = firstText.match(/^\s*\[!([A-Za-z0-9_-]+)\]/);
        if (!calloutMatch) {
          return;
        }
        ...
        icon.textContent = calloutType === 'warning' ? '!' : 'i';
        const label = doc.createElement('span');
        label.className = 'callout-label';
        const labelKey = `${calloutType}Label`;
        label.textContent = ui[labelKey] || calloutType.charAt(0).toUpperCase() + calloutType.slice(1);
        title.append(icon, label);
        const body = doc.createElement('div');
        body.className = 'callout-body';
        stripCalloutToken(firstParagraph, calloutType);
        if (!firstParagraph.textContent?.trim() && firstParagraph.children.length === 0) {
          firstParagraph.remove();
        }
        body.innerHTML = blockquote.innerHTML;
    ...
    function stripCalloutToken(paragraph, calloutType) {
      if (!paragraph) {
        return;
      }
      const tokenRegex = new RegExp(`^\\s*\\[!${calloutType}\\]\\s*`, 'i');
      const walker = paragraph.ownerDocument.createTreeWalker(paragraph, NodeFilter.SHOW_TEXT);
      let node = walker.nextNode();
      while (node) {
        const value = node.nodeValue || '';
        const updated = value.replace(tokenRegex, '');
        if (updated !== value) {
          if (updated) {
    ...
    function extractDescriptionFromContent(container) {
      if (!container) {
        return '';
      }
    ...
    function buildCookbookTitle(doc) {
      const ui = getUiCopy();
      if (!doc) {
        return ui.pageTitle;
      }
      return `${doc.title} | ${ui.pageTitle}`;
    }

    function updateCookbookMeta({ doc, markdownText, descriptionOverride } = {}) {
      const ui = getUiCopy();
      const frontMatter = markdownText ? parseFrontMatter(markdownText) : {};
      const description =
        descriptionOverride ||
        frontMatter.description ||
        extractDescriptionFromContent(content) ||
        ui.pageDescription;
      const title = frontMatter.title || buildCookbookTitle(doc);
      const canonicalRelative = buildCookbookUrl(doc?.slug || '', activeLanguage);
      const canonicalUrl = new URL(canonicalRelative, COOKBOOK_META.siteBaseUrl).toString();
      const image = frontMatter.image
        ? new URL(frontMatter.image, COOKBOOK_META.siteBaseUrl).toString()
        : COOKBOOK_META.defaultImage;
      const themeColor = frontMatter.themeColor || COOKBOOK_META.themeColor;
      ...
      window.ALEPsychMeta?.setOverrides({
        ...
        jsonLdType: doc ? 'WebPage' : 'WebPage',
      });
    ...
    function loadDoc(doc) {
      ...
      if (!doc) {
        ...
        updateCookbookMeta({ doc: null, descriptionOverride: ui.pageDescription });
        return;
      }
      currentDoc = doc;
      currentMarkdown = '';
      ...
      fetch(doc.path)
        ...
        .then((text) => {
          ...
          updateCookbookMeta({ doc, markdownText: text });
        })
        .catch(() => {
    ...
    function setLanguage(lang, { persist = true, updateUrl = true } = {}) {
      ...
      updateCookbookMeta({ doc: currentDoc, markdownText: currentMarkdown });
